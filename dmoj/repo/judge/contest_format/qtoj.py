...
@register_contest_format('qtoj')
class AtCoderContestFormat(DefaultContestFormat):
    name = gettext_lazy('QTOJ')
    config_defaults = {'penalty': -5}
    config_validators = {'penalty': lambda x: x <= 0}
    """
        penalty: Number of points to be deducted for each wrong submission. Defaults to -5.
    """
...
                # Compute penalty
                subs = participation.submissions.filter(problem_id=prob).exclude(submission__result__in=['IE', 'CE'])
                penalty += (subs.count() - 1) * abs(self.config['penalty'])
                if score:
                    cumtime = max(cumtime, dt)

                format_data[str(prob)] = {'time': dt, 'points': score, 'penalty': penalty}
                points += score
    ...
    def get_short_form_display(self):
        ...
        penalty = abs(self.config['penalty'])
        if penalty:
            yield ngettext(
                'Each wrong submission will incur a **penalty of %d point**.',
                'Each wrong submission will incur a **penalty of %d points**.',
                penalty,
            ) % penalty
